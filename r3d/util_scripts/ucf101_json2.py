...
# label_csv_path : classInd.txt
# train_csv_path : trainlist0.txt
# val_csv_path : testlist0.txt
# args.video_path = jpg

def convert_ucf101_csv_to_json(label_csv_path, train_csv_path, val_csv_path,
                               video_dir_path, dst_json_path):
    labels = load_labels(label_csv_path) # classInd.txt
    train_database = convert_csv_to_dict(train_csv_path, 'training') # trainlist0.txt


    val_database = convert_csv_to_dict(val_csv_path, 'validation') # testlist0.txt
...
if __name__ == '__main__':
    ...
    for split_index in range(1, 4):
        ...
        dst_json_path = args.dst_path / 'export_0{}.json'.format(split_index)
